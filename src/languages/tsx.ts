...
/** 1) Types après ':' (évite ternaire) — bloque backticks */
...
function emitTypesAfterColon(
	...
	if (pn >= 0 && input[pn] === "?") return;

	const body = m[1] ?? "";
	const bodyOffset = colonPos + m[0].indexOf(body);
	emitTypeIdentsInRange(input, bodyOffset, bodyOffset + body.length, push);
}

/** 2) Types après `as` — bloque backticks */
...
/** 3) `extends` / `implements` — bloque backticks */
...
export const tsx: Language = extendLanguage(
	jsx,
	"tsx",
	[
		declRule,
